...
如果希望下载结果表格中的文件名支持 **Ctrl+点击在 Obsidian 中打开**，还需要配置 Vault 根目录和名称：
...
> 💡 **原理**：工具会自动计算文件相对于 Vault 根目录的路径，生成 `obsidian://open?vault=...&file=...` 链接，通过终端的 OSC 8 超链接协议实现 Ctrl+点击跳转。
...
- **Obsidian 可点击链接**：下载结果表格中的文件名支持 Ctrl+点击直接在 Obsidian 中打开
- 配置化 Vault 映射：通过 `config.py` 的 `OBSIDIAN_VAULT_ROOT` + `OBSIDIAN_VAULT_NAME` 自动生成 `obsidian://` 链接
- 强制终端模式：`Console(force_terminal=True)` 确保通过启动菜单调用时也能正常输出 OSC 8 超链接

### v0.2.0

- 功能 B：多 P 视频全部分 P 自动下载（`--all-parts` 参数 + 交互式 A/B 选择）
