...
**任务状态**：已完成，核心功能可用，功能 B（多 P 视频全分 P 下载）已追加并推送  
**预估耗时**：2 小时  
...
### 迭代 8：功能 B — 多 P 视频全分 P 自动下载

- **问题**：用户希望输入一个多 P 视频链接后，程序能自动识别分 P 数量并一键下载全部分 P 字幕，保存到以视频标题命名的文件夹
- **根因**：原设计只支持"指定 `?p=N` 下载单个分 P"或"默认下载 P1"，没有"全量下载多 P"的批量路径
- **方案设计**：
  - 新增 `has_explicit_page_param()` 判断 URL 是否显式带 `?p=N`
  - 无显式 `p` 参数且多 P 时，交互模式提示 A/B 选择；非交互模式可通过 `--all-parts / -a` 一键全下
  - 选择 B 后，自动创建 `_safe_folder_name(meta.title)` 子文件夹，每个分 P 独立保存
  - 去重键从 `(bvid, output_dir)` 改为 `(bvid_p{page}, output_dir)`，避免同一视频不同分 P 被合并
- **验证**：BV1Ra5K61EQ4（MIT AI 课程，12 分 P）成功下载 10 个分 P，2 个分 P 因无可用字幕被正确标记 skipped
- **教训**：扩展默认行为时，必须保持原有用户的习惯路径不变；显式参数（`?p=N`）应该具有最高优先级
...
| 功能 B：多 P 视频全分 P 下载 | 自动识别分 P 数量并批量下载到子文件夹 | BV1Ra5K61EQ4 验证 10/12 成功 | 2025-06-10 | 开发者 |
